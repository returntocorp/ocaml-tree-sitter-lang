...
let map_reserved_identifier (x : CST.reserved_identifier) =
  (match x with
  | `Rese_id_open tok -> token tok
  | `Rese_id_modu tok -> token tok
  )

let map_floating_point_type (x : CST.floating_point_type) =
  (match x with
  | `Floa_point_type_float tok -> token tok
  | `Floa_point_type_doub tok -> token tok
  )
...
let map_integral_type (x : CST.integral_type) =
  (match x with
  | `Inte_type_byte tok -> token tok
  | `Inte_type_short tok -> token tok
  | `Inte_type_int tok -> token tok
  | `Inte_type_long tok -> token tok
  | `Inte_type_char tok -> token tok
  )
...
let map_requires_modifier (x : CST.requires_modifier) =
  (match x with
  | `Requis_modi_tran tok -> token tok
  | `Requis_modi_stat tok -> token tok
  )

let rec map_name (x : CST.name) =
  (match x with
  | `Name_id tok -> token tok
...
let map_literal (x : CST.literal) =
  (match x with
  | `Lit_deci_int_lit tok -> token tok
  | `Lit_hex_int_lit tok -> token tok
  | `Lit_octal_int_lit tok -> token tok
  | `Lit_bin_int_lit tok -> token tok
  | `Lit_deci_floa_point_lit tok -> token tok
  | `Lit_hex_floa_point_lit tok -> token tok
  | `Lit_true tok -> token tok
  | `Lit_false tok -> token tok
  | `Lit_char_lit tok -> token tok
  | `Lit_str_lit tok -> token tok
  | `Lit_null_lit tok -> token tok
  )
...
and map_primary (x : CST.primary) =
  ...
  | `Prim_class_lit (v1, v2, v3) ->
      ...
      let v3 = token v3 in
      todo (v1, v2, v3)
  | `Prim_this tok -> token tok
  | `Prim_id tok -> token tok
...
and map_simple_type (x : CST.simple_type) =
  (match x with
  | `Simple_type_void_type tok -> token tok
  | `Simple_type_inte_type x -> map_integral_type x
  | `Simple_type_floa_point_type x ->
      map_floating_point_type x
  | `Simple_type_bool_type tok -> token tok
  | `Simple_type_id tok -> token tok
