...
let map_anon_choice_field_id (env : env) (x : CST.anon_choice_field_id) =
  (match x with
...
let rec map_type_specifier (env : env) (x : CST.type_specifier) =
  ...
  | `Struct_spec (v1, v2, v3) ->
      ...
      in
      let v3 =
        map_anon_choice_field_id_opt_field_decl_list env v3
      in
      todo env (v1, v2, v3)
  | `Union_spec (v1, v2, v3) ->
...
and map_anon_choice_field_id_opt_field_decl_list (env : env) (x : CST.anon_choice_field_id_opt_field_decl_list) =
  (match x with
...
let rec map_non_case_statement (env : env) (x : CST.non_case_statement) =
  (match x with
  | `Labe_stmt (v1, v2, v3) ->
      let v1 = token env v1 (* pattern [a-zA-Z_]\w* *) in
      let v2 = token env v2 (* ":" *) in
