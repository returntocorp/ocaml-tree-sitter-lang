...
external create_parser :
  unit -> Tree_sitter_API.ts_parser = "octs_create_parser_csharp"
...
let rec trans_variable_designation ((kind, body) : mt) : CST.variable_designation =
  match body with
  | Children v ->
      (match v with
      | Alt (0, v) ->
...
and trans_parenthesized_variable_designation ((kind, body) : mt) : CST.parenthesized_variable_designation =
  match body with
  | Children v ->
      (match v with
      | Seq [v0; v1; v2] ->
...
let rec trans_name ((kind, body) : mt) : CST.name =
  match body with
  | Children v ->
      (match v with
      | Alt (0, v) ->
...
let parse_input_tree input_tree =
  ...
  trans_compilation_unit matched_tree
